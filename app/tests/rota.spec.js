...
test.describe('Rota against the mock', () => {
  ...
  test('edit a song in the list: change the BPM, save, the list shows it; remove takes two taps', async ({ page }) => {
    await open(page, `/lead/${LEADER}`, { reset: true })
    await page.getByText(/^Songs \(\d+\)/).click()
    const row = page.locator('li[data-song]').first()
    const id = await row.getAttribute('data-song')
    const title = await row.locator('strong').first().innerText()
    await row.getByRole('button', { name: /^Edit / }).click()
    const bpm = row.getByLabel('BPM'); await bpm.fill('99')
    await row.getByRole('button', { name: 'Save' }).click()
    await expect(page.getByText(/^Saved/).first()).toBeVisible()
    await expect(page.locator(`li[data-song="${id}"]`)).toContainText('99 BPM')
    const row2 = page.locator(`li[data-song="${id}"]`) // the edit form shows the title in an input, so match by id not text
    await row2.getByRole('button', { name: /^Edit / }).click()
    const remove = row2.getByRole('button', { name: /remove/i })
    await remove.click(); await expect(remove).toHaveText('Tap again to remove')
    await remove.click()
